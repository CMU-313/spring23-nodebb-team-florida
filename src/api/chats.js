...
function rateLimitExceeded(caller) {
    ...
    if (
        now - session.lastChatMessageTime <
        meta_1.default.config.chatMessageDelay
    ) {
        return true;
    }
        ...
        yield Promise.all(
            data.uids.map((uid) =>
                __awaiter(this, void 0, void 0, function* () {
                    return messaging_1.default.canMessageUser(caller.uid, uid);
                })
            )
        );
        // The next line calls a function in a module that has not been updated to TS yet
...
function post(caller, data) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        // The next line calls a function in a module that has not been updated to TS yet
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
        const aggregateHook = yield plugins_1.default.hooks.fire(
            "filter:messaging.send",
            {
                data,
                uid: caller.uid,
            }
...
function rename(caller, data) {
    return __awaiter(this, void 0, void 0, function* () {
        // The next line calls a function in a module that has not been updated to TS yet
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
        yield messaging_1.default.renameRoom(
            caller.uid,
            data.roomId,
            data.name
        );
        // The next line calls a function in a module that has not been updated to TS yet
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
        const uids = yield messaging_1.default.getUidsInRoom(
            data.roomId,
            0,
            -1
        );
        ...
        const eventData = {
            roomId: data.roomId,
            newName: validator_1.default.escape(String(data.name)),
        };
        yield helpers_1.default.emitToUids(
            "event:chats.roomRename",
            eventData,
            uids
        );
        // The next line calls a function in a module that has not been updated to TS yet
...
function invite(caller, data) {
    return __awaiter(this, void 0, void 0, function* () {
        // The next line calls a function in a module that has not been updated to TS yet
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
        const userCount = yield messaging_1.default.getUserCountInRoom(
            data.roomId
        );
        // The next line calls a function in a module that has not been updated to TS yet
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
        ...
        // The next line calls a function in a module that has not been updated to TS yet
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
        yield messaging_1.default.addUsersToRoom(
            caller.uid,
            data.uids,
            data.roomId
        );
...
function kick(caller, data) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (
            data.uids.length === 1 &&
            parseInt(data.uids[0], 10) === parseInt(caller.uid, 10)
        ) {
            // The next line calls a function in a module that has not been updated to TS yet
            // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
            ...
            // The next line calls a function in a module that has not been updated to TS yet
            // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
            yield messaging_1.default.removeUsersFromRoom(
                caller.uid,
                data.uids,
                data.roomId
            );
