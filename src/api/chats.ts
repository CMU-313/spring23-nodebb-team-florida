...
function rateLimitExceeded(caller: Caller): boolean {
  const session: Session = caller.request
    ? caller.request.session
    : caller.session; // socket vs req
...
export async function create(caller: Caller, data: Data) {
  ...
  // The next line calls a function in a module that has not been updated to TS yet
  // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
  const roomId: number = (await messaging.newRoom(
    caller.uid,
    data.uids
  )) as number;
...
export async function post(caller: Caller, data: Data) {
  ...
  // The next line calls a function in a module that has not been updated to TS yet
  // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
  const aggregateHook: Hook = (await plugins.hooks.fire(
...
export async function rename(caller: Caller, data: Data) {
  ...
  // The next line calls a function in a module that has not been updated to TS yet
  // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
  const uids: number[] = (await messaging.getUidsInRoom(
    data.roomId,
    0,
    -1
  )) as number[];
  ...
  const eventData: EventData = {
    roomId: data.roomId,
    newName: validator.escape(String(data.name)) as string,
  };
...
export async function invite(caller: Caller, data: Data) {
  // The next line calls a function in a module that has not been updated to TS yet
  // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
  const userCount: number = (await messaging.getUserCountInRoom(
    data.roomId
  )) as number;
  // The next line calls a function in a module that has not been updated to TS yet
  // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
