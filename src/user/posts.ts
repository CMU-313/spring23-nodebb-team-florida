...
export = function (User: user) {
  ...
  async function isReady(uid: string, cid: string[], field: string) {
    ...
    // The next line calls a function in a module that has not been updated to TS yet
    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
    const newbiepostdelaythreshold: number = meta.config
      .newbiePostDelayThreshold as number;
  ...
  async function incrementUserFieldAndSetBy(
    uid: string,
    field: string,
    set: string,
    valueStr: string
    ...
    // The next line calls a function in a module that has not been updated to TS yet
    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
    const newValue = (await User.incrementUserFieldBy(
      uid,
      field,
      value
    )) as string;
